...
class MessUser:
    ...
    def login(self):
        ...
        by doing one of the follwing
        - loading the cookies
        - asking password and authenticating
        """
        if os.path.isfile(self.cookie_file):
            # sets session cookies to login cookies when the cookie file exists
            with open(self.cookie_file, 'rb') as f:
                self.session.cookies = pickle.load(f)
            self.logger.info(f"Using existing login cookie {self.cookie_file}")
        else:
            # if cookie file not present prompt password
            password = getpass(f"Password for {self.user_id}: ")
            # send login request
    ...
    def _login(self, password) -> None:
        """
        Logs in the user with the username password
...
def main():
    ...
    subparsers = parser.add_subparsers(
                 ...
                 dest='command',
                 required=True)
    subparser_user_report = subparsers.add_parser(
                            'userreport',
